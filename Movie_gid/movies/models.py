...
class FilmDirector(models.Model):
    ...
    age = models.PositiveSmallIntegerField('Возраст', default=0)
    description = models.TextField('Описание')
    image = models.ImageField('Изображение', upload_to='FilmDirectors/')

    def __str__(self):
        return self.name

    def get_absolute_url(self):
        return reverse('film_director_detail', kwargs={'slug': self.name})
...
class Movie(models.Model):
    ...
    directors = models.ManyToManyField(FilmDirector, verbose_name='режиссер', related_name='film_director')
